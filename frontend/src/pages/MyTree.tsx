...
function MyTree() {
  ...
  const fetchCounts = async () => {
    ...
    const emailsResponse = await fetch(
      `${import.meta.env.VITE_API_URL}/emails/count`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    const pdfResponse = await fetch(
      `${import.meta.env.VITE_API_URL}/pdfs/count`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    const eventResponse = await fetch(
      `${import.meta.env.VITE_API_URL}/events/count`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const [smallInfo, setSmallInfo] = useState<any>(
    "Feel free to click on any moment and learn more about what makes you you."
  );
  const [underlineColor, setUnderlineColor] = useState<any>(
  ...
  const handleUpload = async (event: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/pdfs/upload-pdf`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          ...
          <form onSubmit={handleUpload}>
            <div className="form-control w-full">
              ...
              <button
                ...
                className={`btn bg-accent-content shadow-sm mt-4 w-full ${
                  selectedFile && "text-base-200"
                }`}
              >
      ...
      <div className="h-[calc(100vh-64px)] w-screen flex">
        ...
        <div className="flex-3 border-l border-gray-300  overflow-y-scroll">
          ...
          <div className="px-6 py-5">
            ...
            <div className="space-y-4 pl-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-semibold">{pdfCount}</span> uploaded
                  documents
                </div>
              </div>
            </div>
            <button
              className="btn btn-outline btn-block mt-5 p-5"
              onClick={() => {
          ...
          <div className="px-6 py-5">
            ...
            <button className="btn btn-block bg-accent-content text-base-200 p-5">
              Share Timeline
            </button>
          </div>
        </div>
